    ...
    >
        ...
        <div className="grid gap-8 lg:grid-cols-[1.2fr_1fr] lg:items-stretch lg:gap-12">
          ...
          >
            ...
            <h2 className="mt-5 font-heading text-3xl font-extrabold leading-[1.15] tracking-tight text-foreground sm:text-4xl lg:text-[2.75rem]">
              Turning local{" "}
              <span className="text-[hsl(var(--trust-blue))]">
                resourcefulness
              </span>{" "}
            ...
            <p className="mt-4 text-sm italic text-muted-foreground/90">
          ...
          >
            ...
            <div className="absolute inset-x-4 bottom-4 rounded-2xl border border-white/20 bg-[hsl(var(--navy))]/65 p-4 backdrop-blur-md sm:inset-x-5 sm:bottom-5 sm:p-5">
              <dl className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
                {QUICK_FACTS.map((f) => (
                  <div key={f.label}>
                    <dt className="text-[10px] font-semibold uppercase tracking-[0.14em] text-white/70">
                      {f.label}
                    </dt>
                    <dd className="mt-1 font-heading text-sm font-extrabold leading-tight text-white">
                      {f.value}
                    </dd>
                  </div>
                ))}
      ...
      <section className="mt-10 grid gap-4 lg:grid-cols-2">
        <article className="rounded-2xl bg-[hsl(var(--trust-blue))] p-6 text-white shadow-[var(--shadow-card)] transition-transform hover:-translate-y-1">
          ...
          <h3 className="mt-2 font-heading text-xl font-bold">
            Turning resourcefulness into opportunity.
          </h3>
      ...
      <section className="mt-14">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <Link
            to="/programs"
            className="inline-flex items-center gap-1 text-sm font-semibold text-[hsl(var(--trust-blue))] hover:underline"
          >
        ...
        <ol className="relative mt-8 grid gap-3 md:grid-cols-5">
          <div
            className="absolute left-4 right-4 top-5 hidden h-px bg-gradient-to-r from-[hsl(var(--trust-blue))] via-[hsl(var(--amber))] to-[hsl(var(--trust-blue))] md:block"
            aria-hidden
          />
          {approach.map((s, i) => {
      ...
      <section className="mt-14 grid gap-4 md:grid-cols-2">
        ...
        >
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Seattle, Washington and Ethiopia. Regions expand only when
              partnerships and permissions are documented.
            </p>
        ...
        >
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              A founding board of three directors provides oversight and
              safeguards mission alignment.
            </p>
