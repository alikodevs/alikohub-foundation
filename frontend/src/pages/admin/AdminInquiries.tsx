...
import { useInquiriesList, useUpdateInquiry } from "@/hooks/useInquiries";
...
export default function AdminInquiries() {
  const { data: rawInquiries, isLoading: loading } = useInquiriesList();
  const updateMutation = useUpdateInquiry();

  const rows: Inquiry[] = useMemo(() => {
    return ((rawInquiries || []) as unknown as Record<string, unknown>[]).map((item) => ({
      id: String(item.id),
      inquiry_type: (item.inquiryType || item.inquiry_type || "general") as InquiryType,
      name: String(item.name || ""),
      email: String(item.email || ""),
      organization: (item.organization as string) || null,
      message: String(item.message || ""),
      source_page: (item.sourcePage as string) || (item.source_page as string) || null,
      status: (item.status as InquiryStatus) || "new",
      admin_notes: (item.adminNotes as string) || (item.admin_notes as string) || null,
    ...
    updateMutation.mutate(
      {
        ...
        data: {
          status: draftStatus,
          adminNotes: draftNotes || undefined,
        },
      },
      {
        onSuccess: () => {
