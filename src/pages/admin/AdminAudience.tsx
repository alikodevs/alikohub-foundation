...
function SubscribersPanel() {
  const { data: subscribers = [], isLoading } = useCrmList<NewsletterSubscriber>(
    "newsletter_subscribers",
  );
  const { save, remove } = useCrmMutations("newsletter_subscribers", "Subscriber");
  ...
  const filtered = useMemo(() => {
    ...
    return subscribers.filter((s) => {
      const matchesTerm =
        !term || s.email.toLowerCase().includes(term) || (s.name ?? "").toLowerCase().includes(term);
  ...
  function exportCsv() {
    const rows = [
      ...
      ...filtered.map((s) => [s.email, s.name ?? "", s.status, s.source_page ?? "", s.created_at]),
    ];
            ...
            <TableBody>
              {filtered.map((sub) => (
                <TableRow key={sub.id}>
                  ...
                  <TableCell>
                    <Badge variant={sub.status === "subscribed" ? "default" : "outline"}>
                  ...
                  <TableCell className="text-right">
                    {sub.status === "subscribed" ? (
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          save.mutate({ id: sub.id, values: { status: "unsubscribed" } })
                        }
                      >
                        Unsubscribe
                      </Button>
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          save.mutate({ id: sub.id, values: { status: "subscribed" } })
                        }
                      >
                        Resubscribe
                      </Button>
                    ...
                    <Button
                      ...
                      size="icon"
                      onClick={() => {
                        if (confirm(`Remove ${sub.email}?`)) remove.mutate(sub.id);
                      }}
                    >
                      <Trash2 className="h-4 w-4 text-destructive" />
                    </Button>
